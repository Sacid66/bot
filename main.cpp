// Main.cpp
// Tek dosya Vulkan + GLFW örneği — runtime GLSL -> SPIR-V derlemek için shaderc kullanır.
// Derleme örneği (Linux): 
// g++ Main.cpp -std=c++17 -O2 -o vulkan_triangle `pkg-config --cflags --libs glfw3` -lshaderc -lvulkan -ldl -pthread
...
// Very small helper: check extension support for GLFW Vulkan surface
...
private:
    ...
    // Buffers (minimal: CPU visible vertex buffer)
    ...
    // --- Vulkan init helpers (simplified) ---
    void initVulkan() {
        createInstance();
        setupDebugMessenger();
        createSurface();
    ...
    bool isDeviceSuitable(VkPhysicalDevice dev) {
        ...
        // swapchain support (minimal)
        bool extensionsOK = checkDeviceExtensionSupport(dev);
        bool swapchainOK = false;
        if (extensionsOK) {
            // query formats & present modes
            swapchainOK = true; // for minimal example assume OK; detailed checks omitted
        }
    ...
    // --- Swapchain (very simplified) ---
    struct SwapChainSupportDetails {
        // minimal - not fully fleshed out
    };

    void createSwapChain() {
        // For brevity: create a very simple swapchain with default format/present mode.
    ...
    // Compile GLSL -> SPIR-V at runtime using shaderc
    ...
    void createGraphicsPipeline() {
        // Simple vertex and fragment shader sources
...
void main() {
    outColor = vec4(fragColor, 1.0);
        ...
        auto vertSPV = compileShader(vertShaderGLSL, shaderc_vertex_shader, "triangle.vert");
        auto fragSPV = compileShader(fragShaderGLSL, shaderc_fragment_shader, "triangle.frag");
    ...
    void createVertexBuffer() {
        ...
        // For simplicity: create host visible buffer and map it (not optimal but simplest)
